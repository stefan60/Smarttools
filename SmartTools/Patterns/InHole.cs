...
namespace SmartTools.Patterns {
  /// <summary>създава модел на избушен отвор навътре</summary>
  [ComVisible(true)]
  public class InHole: Lib.PatternBase {
    ...
    public override Prime.Patterns_e modelType {
      get { return Prime.Patterns_e.InHole; }
    }
    ...
    /// <summary>стойност по подразбиране за отстояниетоа на отвора от центъра</summary>
    public double defDistance { get { return Prime.TEST ? 0.5 : 0; } }
    ...
    public override void GetCoordinateSystem() {
      ...
      this.FilePropetyTab();
      this.holeNumber = this.fileProperties.InHoleNumber() + 1;
    ...
    public override bool Creating() {
      ...
      if (this.diameter > 0 && isOk) {
        ...
        if (this.pattern.Equals(Prime.Patterns_e.Mantel.ToString().ToLower())) {
          this.ClearSelection();
          this.Selected(this.namePlaneHole, this.swPlane, false, 1);
    ...
    public override void SetFileProperties() {
      this.FilePropetyTab();
      this.fileProperties.InHoleNumber(this.holeNumber);
      this.fileProperties.Diameter(this.diameter, "_inHole" + this.holeNumber.ToString());
      this.fileProperties.Height(this.height, "_inHole" + this.holeNumber.ToString());
      this.fileProperties.Angle(this.angularOffset, "_inHole" + this.holeNumber.ToString());
    } // SetFileProperties

    /// <summary>създава папка с постоенията</summary>
    public override void CreateFolder() {
      if (this.GetFeature("ИзВтОтвор" + this.holeNumber.ToString()) == null) {
        ...
        this.swDocument.SelectedFeatureProperties(0, 0, 0, 0, 0, 0, 0, true, false, "ИзВтОтвор" + this.holeNumber.ToString());
    ...
    /// <summary>създава помощна повърхнина на отстояние 10mm от вътрешната стена на тялото</summary>
    protected void CreateInternalSurface() {
      if (this.pattern.Equals(Prime.Patterns_e.Mantel.ToString().ToLower())) {
        Mantel mantel = (Mantel)this.GetPatternByName(this.pattern);
        mantel.CreateSurface(-0.01, false, this.nameInternalSurface, this.nameInternalSurfaceSketch);
      } else {
        ...
        bool status = this.Selected("Line1@" + namePattern + "Sketch", this.swExtSketchSegment);
        status = this.Selected("Arc1@" + namePattern + "Sketch", this.swExtSketchSegment, true);
        ...
        this.ClearSelection();
        status = this.Selected(this.nameInternalSurfaceSketch, this.swSketch);
        status = this.Selected(namePattern + "Axis", this.swAxis, true, 16);
        this.swFeatureManager.FeatureRevolve2(true, false, false, false, false, false,
          (int)swEndConditions_e.swEndCondBlind,
          0, Prime.RADIANS_360DEG, 0, false, false, 0, 0, 0, 0, 0, true, true, true);
        this.RenameFeature("Surface-Revolve", this.nameInternalSurface);
      }
    } // CreateInternalSurface

    /// <summary>създава помощна повърхнина по външната стена на тялото</summary>
    protected void CreateExternalSurface() {
      if (this.pattern.Equals(Prime.Patterns_e.Mantel.ToString().ToLower())) {
        Mantel mantel = (Mantel)this.GetPatternByName(this.pattern);
        mantel.CreateSurface(0, true, this.nameExternalSurface, this.nameExternalSurfaceSketch);
      } else {
        string namePattern = Prime.Patterns_e.Bottom.ToString().ToLower();
        double thin = -this.thinBody;
        if (this.pattern.Equals(Prime.Patterns_e.Cover.ToString().ToLower())) {
          namePattern = Prime.Patterns_e.Cover.ToString().ToLower();
          thin = this.thinBody;
        }
        ...
        this.ClearSelection();
        status = this.Selected(this.nameExternalSurfaceSketch, this.swSketch);
        status = this.Selected(namePattern + "Axis", this.swAxis, true, 16);
        this.swFeatureManager.FeatureRevolve2(true, false, false, false, false, false,
          (int)swEndConditions_e.swEndCondBlind,
          0, Prime.RADIANS_360DEG, 0, false, false, 0, 0, 0, 0, 0, true, true, true);
    ...
    protected void CreatingSurfaces() {
      if (this.pattern.Equals(Prime.Patterns_e.Mantel.ToString().ToLower())) {
        this.SelectPlane(this.swTopPlane);
        // изгражда помощна повърхнина на отстояние 10mm от вътрешната стена на тялото
        this.Circle(0, 0, 0, this.diamBody - 0.02, this.nameDiamBody, this.nameInternalSurfaceSketch);
        this.swSketchManager.InsertSketch(true);
        this.ClearSelection();
        ...
        // изгражда момощна повърхнина по външната стена
        this.ClearSelection();
        this.SelectPlane(this.swTopPlane);
        this.Circle(0, 0, 0, this.diamBody + 2 * this.thinBody, this.nameDiamBody, this.nameExternalSurfaceSketch);
        this.swSketchManager.InsertSketch(true);
        this.ClearSelection();
        ...
        // изгражда момощна повърхнина на отстояние 10mm от вътрешната стена
        this.SelectPlane(this.swFrontPlane);
        ...
        this.RenameFeature("Surface-Revolve", this.nameInternalSurface);
        // изгражда помощна повърхнина по външната стена на тялото
        this.SelectPlane(this.swFrontPlane);
        status = this.swExtension.SelectByID2("Line1@" + namePattern + "Sketch", "EXTSKETCHSEGMENT", 0, 0, 0, true, 0, null, (int)swSelectOption_e.swSelectOptionDefault);
        status = this.swExtension.SelectByID2("Arc1@" + namePattern + "Sketch", "EXTSKETCHSEGMENT", 0, 0, 0, true, 0, null, (int)swSelectOption_e.swSelectOptionDefault);
